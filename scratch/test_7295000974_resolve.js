...
async function main() {
  ...
  console.log("Starting simulation for 7295000974 info resolution...");
  ...
  if (ftcInfo) {
    ...
    await pool.query(`
      UPDATE businesses 
      SET b_nm = $1, b_adr = $2, p_nm = $3, b_sector = $4, brand_name = $5
      WHERE b_no = $6
    `, [ftcInfo.cmpNm, ftcInfo.repAddr, ftcInfo.rprsNm, '전자상거래 소매업 (통신판매업)', `${ftcInfo.cmpNm}, ${ftcInfo.cmpNm}`, bNo]);
    ...
    console.log("Post-simulation brand_name in DB:", postRes.rows[0]?.brand_name);
