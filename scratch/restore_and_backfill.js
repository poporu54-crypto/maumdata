...
// ⚠️ 버그 수정: bizrNo 파라미터를 bzno로 정정하여 공공 API 첫번째 행(영문펀드)으로 오염 적재되는 문제를 원천 차단합니다.
async function getCorpBasicOutline(bNo) {
  const cleanBNo = bNo.replace(/[^0-9]/g, "");
  const url = `https://apis.data.go.kr/1160100/service/GetCorpBasicInfoService_V2/getCorpOutline_V2?serviceKey=${DATA_PORTAL_SERVICE_KEY}&pageNo=1&numOfRows=1&resultType=json&bzno=${cleanBNo}`;
  try {
...
async function main() {
  ...
  // Step 1: 로컬 businesses.json을 기반으로 모든 기업의 원본 올바른 정보로 원복시킵니다.
  const localDbPath = path.join(__dirname, '../src/data/businesses.json');
  const raw = fs.readFileSync(localDbPath, 'utf8');
  const localBusinesses = JSON.parse(raw);

  console.log("Step 1: Restoring contaminated DB fields from businesses.json backup...");
  for (const biz of localBusinesses) {
    ...
    const normalizedBiz = {
      ...
      b_adr: biz.b_adr,
      b_sector: biz.b_sector,
      b_type: biz.b_type || "",
      corp_no: biz.corp_no || "",
      dart_code: biz.dart_code || "",
      description: biz.description || "",
      credit_rating: biz.credit_rating || "",
      industry_rank: biz.industry_rank || "",
      is_sme: biz.is_sme || "",
      listing_status: biz.listing_status || "",
      homepage: biz.homepage || "",
      main_biz: biz.main_biz || "",
      is_audited: biz.is_audited || false,
      npsSbscrbNmps: biz.npsSbscrbNmps || 0,
      npsLinked: biz.npsLinked || false,
      history: biz.history || [],
      dataSource: "local"
    };

    await dbUpsertBusiness(normalizedBiz);
    console.log(`-> Restored ${biz.b_nm} (${cleanBNo}) successfully.`);
  }

  // Step 2: 올바른 API 파라미터 bzno를 이용해 최신 공공 API 데이터를 다시 안정적으로 수집 및 채워넣습니다.
  console.log("\nStep 2: Re-running backfill with correct API param 'bzno'...");
  
  // DB에 존재하는 정상 등록 기업 목록 다시 추출 (오염이 풀린 지윤 주식회사 및 토스 등으로 복원된 상태)
  ...
  for (let dbBiz of businesses) {
    ...
    if (basicInfo) {
      ...
      business = {
        b_no: cleanBNo,
        // 상호명이 영문펀드명으로 변환되는 것을 방지하기 위해 로컬 DB명 우선시
        b_nm: basicInfo.corpNm && !basicInfo.corpNm.includes("SAMPO FUND") ? basicInfo.corpNm : dbBiz.b_nm,
        ...
        description: dbBiz.description || `${dbBiz.b_nm}은(는) 금융위원회 공시 정보가 등록된 대한민국 공식 ${scale}입니다.`,
  ...
  console.log("\n==================================================");
  console.log("DB Restoration & Backfill finished successfully!");
}

main()
  .catch(err => {
    console.error("Restoration & Backfill failed:", err);
  })
  .finally(async () => {
    await client.end();
  });
