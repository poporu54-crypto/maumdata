...
export async function getUnifiedBusinessData(bNo: string): Promise<{
  ...
  // DB 캐시가 이미 존재하고 유효한 기업명이 있는 경우 ➔ 누락된 상세 정보가 있다면 최초 1회 실시간 동기화 수행 후 서빙
  if (localBiz && localBiz.b_nm !== "상호 정보 없음") {
    const uselessSectors = ["상장 법인", "상장법인", "대기업", "중소기업", "일반기업", "중견기업", "기타 서비스업", "기타서비스업", "미등록 업종", "미등록업종", "-", ""];
    const currentSector = (localBiz.b_sector || "").trim();
    const isUselessSector = uselessSectors.includes(currentSector);

    // 상세 정보가 한번도 수집되지 않았거나(null), 업종이 무의미한 분류인 경우 ➔ 1회성 실시간 외부 API 동기화 개시
    const anySyncAt = (localBiz as any).patents_last_sync_at || (localBiz as any).dart_last_sync_at || (localBiz as any).bids_last_sync_at;
    const isDetailNeverSynced = !anySyncAt || isUselessSector;

    if (isDetailNeverSynced) {
      console.log(`[Database Hit but Details Missing/Useless Sector] Syncing details for existing business: ${localBiz.b_nm} (${cleanBNo})`);
      const dartCode = (localBiz as any).dart_code || "";
      
      // 국민연금 실시간 동기화 및 업종명 보정
      let npsInfo = null;
      try {
        npsInfo = await getNpsBplcInfo(cleanBNo, localBiz.b_nm);
      } catch (err) {
        console.error("[NPS sync error in missing details sync]", err);
      }

      let updatedSector = currentSector;
      let updatedMainBiz = localBiz.main_biz || "";
      let npsFields: any = {};
      ...
      if (npsInfo && npsInfo.npsSbscrbNmps > 0) {
        npsFields = {
          nps_linked: true,
          npsLinked: true,
          nps_sbscrb_nmps: npsInfo.npsSbscrbNmps,
          npsSbscrbNmps: npsInfo.npsSbscrbNmps,
          new_acqs_nmps: npsInfo.newAcqsNmps,
          newAcqsNmps: npsInfo.newAcqsNmps,
          loss_sbscrb_nmps: npsInfo.lossSbscrbNmps,
          lossSbscrbNmps: npsInfo.lossSbscrbNmps,
          nps_chrg_amt: npsInfo.npsChrgAmt,
          npsChrgAmt: npsInfo.npsChrgAmt
        };

        if (npsInfo.npsSector && (!currentSector || uselessSectors.includes(currentSector))) {
          updatedSector = npsInfo.npsSector;
          if (!updatedMainBiz || uselessSectors.includes(updatedMainBiz.trim())) {
            updatedMainBiz = npsInfo.npsSector;
          }
        }
      }
      ...
      await Promise.all([
        syncRecentBidsByCompany(localBiz.b_nm, cleanBNo)
          .catch(err => console.error("[Bids sync error in missing details sync]", err)),
        syncPatentsByCompany(localBiz.b_nm, localBiz.p_nm || "")
          .catch(err => console.error("[Patents sync error in missing details sync]", err)),
        dartCode 
          ? syncDisclosuresByCompany(cleanBNo, dartCode)
              .catch(err => console.error("[DART sync error in missing details sync]", err))
          : Promise.resolve()
      ]).catch(err => console.error("[Detail Sync failed in missing details sync]:", err));

      // DB 갱신
      const cachedBiz = {
        ...localBiz,
        ...npsFields,
        b_sector: updatedSector,
        main_biz: updatedMainBiz,
        dataSource: "local" as const,
        ntsLastSyncAt: (localBiz as any).nts_last_sync_at || new Date(),
        npsLastSyncAt: new Date(),
        patents_last_sync_at: new Date(),
        bids_last_sync_at: new Date(),
        dart_last_sync_at: dartCode ? new Date() : null
      };

      await upsertBusiness(cachedBiz);
      console.log(`[Cache Write] Updated missing details for existing business: ${localBiz.b_nm} (${cleanBNo})`);
      ...
      return {
        apiStatus: {
          b_no: cleanBNo,
          b_stt: finalBiz?.bStt || (finalBiz as any)?.b_stt || "계속사업자",
          b_stt_cd: finalBiz?.bSttCd || (finalBiz as any)?.b_stt_cd || "01",
          tax_type: finalBiz?.taxType || (finalBiz as any)?.tax_type || "부가가치세 일반과세자",
          tax_type_cd: finalBiz?.taxTypeCd || (finalBiz as any)?.tax_type_cd || "01",
          end_dt: finalBiz?.closeDate || (finalBiz as any)?.close_date || "",
          utcc_yn: "N",
          tax_type_change_dt: "",
          invoice_apply_dt: "",
          rbf_tax_type: "",
        ...
        business: finalBiz || (cachedBiz as any),
        isInvalid: false
      };
    }
